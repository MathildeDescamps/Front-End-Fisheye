...
function mediaFactory(data) {
    ...
    if(image){
        ...
        reaWrapper.innerHTML = `
            <div style="background: url('${src}');" class="realisation-image"></div>
        `;
    } 
    if(video){
        let src = `assets/medias/${video}`;
        reaWrapper.innerHTML = `
            <video class="realisation-video" height="300px" controls>
    ...
    reaInfos.innerHTML = `
        ...
        <div class="rea-likes">
            <span class="likes-nb">${likes}</span>
    ...
    reaWrapper.addEventListener("click", function() {
        if(image) {
